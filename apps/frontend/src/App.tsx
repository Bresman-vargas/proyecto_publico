...
import { AdminRoute } from "./components/AdminRoute"; // Importamos tu ruta admin
...
function AppRouter() {
  ...
  // 1. Separamos públicas y privadas
  const publicRoutes = routes.filter((r) => !r.private);
  const privateRoutes = routes.filter((r) => r.private);

  // 2. Sub-clasificamos las privadas según requieran rol admin o no
