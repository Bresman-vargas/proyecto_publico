...
interface SidebarProps {
  expanded: boolean;
  setExpanded: (val: boolean | ((curr: boolean) => boolean)) => void;
}
...
export default function Sidebar({ expanded, setExpanded }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`
          fixed inset-y-0 left-0 bg-bg border-r border-border
          lg:relative lg:translate-x-0
          ${expanded ? "translate-x-0 w-80" : "-translate-x-full lg:translate-x-0 lg:w-20"}
        `}
      >
        <header className="border-b border-border h-15 flex items-center justify-between px-4">
              ...
              <div className="text-txt">
                ...
                >
                  <g id="logotype">
                    ...
                    <path d="M154.857 10.4573C158.843 10.4573 162.086 13.6995 162.086 17.7125C162.086 21.6989 158.843 24.9411 154.857 24.9411H136.254V29.0604H132.135V10.4573H154.857ZM154.83 20.7953C156.558 20.7953 157.94 19.4133 157.94 17.7125C157.94 15.9851 156.558 14.6031 154.83 14.6031H136.254V20.7953H154.83Z"></path>
                    <path d="M109.975 14.6031C107.105 14.6031 104.793 16.9152 104.793 19.7588C104.793 22.629 107.105 24.9411 109.975 24.9411H125.336V21.6989H109.576V17.5796H129.482V29.0604H109.975C104.819 29.0604 100.673 24.9145 100.673 19.7588C100.673 14.6297 104.819 10.4573 109.975 10.4573H129.482V14.6031H109.975Z"></path>
                    <path d="M74 29.0604V10.4573H78.1193V24.9411H98.9281V29.0604H74Z"></path>
                    <path d="M238.311 9.41768C238.311 10.7529 237.228 11.8354 235.893 11.8354C234.558 11.8354 233.475 10.7529 233.475 9.41768C233.475 8.08243 234.558 7 235.893 7C237.228 7 238.311 8.08243 238.311 9.41768Z"></path>
                  </g>
                </svg>
              </div>
            </Link>
          ...
          <div className={`lg:flex ${expanded ? "justify-end" : "justify-center"}`}>
            <button
              onClick={() => setExpanded((curr) => !curr)}
              className="size-10 p-2 bg-bg rounded-md border border-border cursor-pointer"
            >
          ...
          <SidebarContext.Provider value={{ expanded }}>
            <nav className="p-4 flex items-start flex-col gap-4">
              ...
              <SidebarItem
                ...
                label="My Comments"
                link="/user-comments"
                active={location.pathname === "/user-comments"}
              />
...
function SidebarItem({ ico: Icon, label, active = false, link }: ItemPros) {
  ...
  return (
    ...
    >
      <Icon />
      <span className={`overflow-hidden transition-all ${expanded ? "w-52 ml-3" : "hidden"}`}>
        {label}
      </span>
    </Link>
  );
